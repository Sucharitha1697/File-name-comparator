...
class GFG implements Comparator<String>{
	...
	{
	    //to check if both or one of the strings is empty
	     //If one string empty then the other string will be the larger one
	     //if both are empty then both the string are equal
		...
		if (j<len2) 
		{
		    ...
		    if(!val && checkdig(str2.charAt(i))==1)
		        return -1;
	    }
		
		//for cases like kloh300t,kloh500,here kloh300tis smaller
		if(ans!=0)
		    return ans;
